...
admin.initializeApp({
  ...
  databaseURL:
    "https://disneyplus-clone-db312-default-rtdb.asia-southeast1.firebasedatabase.app",
});
...
for (const movieId in moviesData.movies) {
  const movie = moviesData.movies[movieId];

  moviesCollectionRef
    .doc(movieId)
    .set(movie)
    .then(function (docRef) {
      console.log("Document written:", movieId);
    })
    .catch(function (error) {
      console.error("Error adding document:", movieId, error);
    });
}
